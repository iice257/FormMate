...
export function workspaceScreen() {
  ...
  const html = `
    <div class="h-screen bg-slate-50 text-slate-900 flex flex-col font-sans dark-mode-transition overflow-hidden">
      <!-- Header -->
      <header class="h-16 lg:h-18 glass-header flex items-center justify-between px-4 lg:px-6 shrink-0 z-30 shadow-sm relative">
        <div class="flex items-center gap-3">
          <div id="sidebar-logo" class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary-light text-white flex items-center justify-center font-bold text-xl shadow-primary cursor-pointer btn-press">
            F
          </div>
          <div class="hidden sm:flex flex-col">
            <span class="font-black text-lg tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 leading-none">FormMate</span>
        ...
        <div class="hidden md:flex flex-1 max-w-md mx-6">
          <button id="btn-search-cmd" class="w-full flex items-center justify-between gap-2 px-4 py-2 bg-slate-100/80 hover:bg-slate-200/80 text-slate-500 rounded-xl text-sm transition-colors border border-slate-200 backdrop-blur-sm group">
            <div class="flex items-center gap-2">
              <span class="material-symbols-outlined text-sm text-slate-400 group-hover:text-primary transition-colors">search</span>
              <span>Search Copilot...</span>
            </div>
            <div class="flex gap-1">
              <kbd class="font-mono text-[10px] bg-white px-1.5 py-0.5 rounded shadow-sm text-slate-400 font-bold border border-slate-200">Cmd</kbd>
              <kbd class="font-mono text-[10px] bg-white px-1.5 py-0.5 rounded shadow-sm text-slate-400 font-bold border border-slate-200">K</kbd>
            </div>
          </button>
        </div>

        <div class="flex items-center gap-2 lg:gap-4">
          <button id="btn-toggle-chat" class="md:hidden p-2 text-slate-500 hover:text-primary transition-colors hover:bg-slate-100 rounded-lg">
            <span class="material-symbols-outlined">forum</span>
          </button>
          
          <button id="btn-review" class="hidden md:flex bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-sm tracking-wide hover:bg-slate-800 transition-all shadow-[0_4px_12px_rgba(15,23,42,0.15)] hover:shadow-[0_8px_16px_rgba(15,23,42,0.2)] active:scale-95 items-center gap-2">
          ...
          <button class="w-10 h-10 rounded-full bg-gradient-to-tr from-slate-100 to-slate-200 border border-slate-300 flex items-center justify-center hover:ring-2 hover:ring-primary/20 transition-all cursor-pointer">
      ...
      <main class="flex-1 flex overflow-hidden lg:pl-2">
        ...
        <aside id="sidebar" class="w-16 lg:w-[260px] glass-panel border border-slate-200/50 rounded-[var(--fm-card-radius)] flex flex-col py-4 shrink-0 transition-all z-20 hidden md:flex my-4 ml-4 shadow-sm" style="height: calc(100% - 2rem);">
          <nav class="flex-1 px-3 space-y-1.5">
            ...
            <div class="my-4 border-t border-slate-100 w-full"></div>
            <button id="nav-vault" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 hover:bg-white hover:shadow hover:text-slate-900 transition-all group">
              <span class="material-symbols-outlined text-slate-400 group-hover:text-primary transition-colors text-[20px]">lock</span>
              <span class="font-semibold text-[14px] hidden lg:block tracking-wide">My Vault</span>
            </button>
          ...
          <div class="mt-auto px-4 hidden lg:block">
            <div class="bg-gradient-to-b from-white to-slate-50 border border-slate-200/60 rounded-xl p-4 shadow-sm relative overflow-hidden group">
              ...
              <div class="flex justify-between items-end text-xs font-bold text-slate-900 mb-2 relative z-10">
                <span class="text-slate-600">Free Plan</span>
                <span class="text-primary text-sm flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">bolt</span> 3<span class="text-slate-400 text-xs font-medium">/5</span></span>
              </div>
              ...
              <button class="mt-4 w-full py-2 bg-slate-100 hover:bg-primary hover:text-white rounded-lg text-[11px] uppercase tracking-widest font-bold text-slate-600 transition-all">Upgrade Pro</button>
            </div>
          </div>
        </aside>
        ...
        <div class="flex-1 flex overflow-hidden relative" id="editor-container">
          ...
          <div class="flex-1 overflow-y-auto relative scroll-smooth no-scrollbar" id="editor-scroll">
            <div class="max-w-3xl mx-auto px-4 lg:px-12 py-8 lg:py-12 pb-32">
              ...
              <div class="mb-10 animate-screen-enter">
                <div class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[11px] font-bold tracking-wide text-slate-500 mb-5 shadow-sm">
                  <span class="material-symbols-outlined text-[14px] text-primary">description</span>
                  ${formData.source || 'Scraped Form'}
                </div>
                ...
                <div class="flex items-center gap-3 mt-8">
                  ...
                  <div class="flex items-center gap-1.5 text-[13px] text-slate-500 bg-slate-100/80 px-3 py-2 rounded-lg font-medium">
              ...
              <!-- Bottom Review CTA (Sticky Overlay) -->
              <div class="fixed bottom-0 left-0 md:left-64 lg:left-72 right-0 lg:right-96 bg-gradient-to-t from-slate-50 via-slate-50/95 to-transparent pt-16 pb-6 px-4 md:px-8 z-10 pointer-events-none dark-mode-transition transition-all">
                <div class="max-w-[42rem] mx-auto pointer-events-auto">
                  <button id="btn-review-bottom" class="w-full bg-slate-900 text-white h-14 rounded-[var(--fm-card-radius)] font-bold text-base shadow-[0_10px_40px_rgba(15,23,42,0.2)] hover:bg-slate-800 hover:shadow-[0_15px_40px_rgba(15,23,42,0.3)] hover:-translate-y-1 transition-all flex items-center justify-center gap-2 btn-press group">
                    <span class="material-symbols-outlined group-hover:scale-110 group-hover:text-primary-light transition-all">check_circle</span>
                    Review & Auto-Fill Form
                  </button>
                </div>
              </div>
  ...
  function init(wrapper) {
    ...
    async function sendMessage(text) {
      ...
      try {
        ...
        aiBubble.innerHTML = `
          <div class="max-w-[90%] bg-slate-50 border border-slate-100 rounded-[var(--fm-card-radius)] rounded-tl-none p-4 text-[13px] text-slate-700 leading-relaxed relative">
