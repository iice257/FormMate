...
- Screenshots are a first-class intake path: users can upload them by themselves, or include a URL as optional source context.
...
- Zen mode controls should stay compact and route-aware: menu hover may reveal the label only while closed, back appears only when an app-shell previous page exists, back never exits to landing, and the zen account group should be Settings-only.
